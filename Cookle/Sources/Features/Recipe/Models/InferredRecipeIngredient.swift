...
public struct InferredRecipeIngredient {
    ...
    @Guide(
        description: """
        Reusable base ingredient name. Keep preparation qualifiers with the amount and retain an A/B group marker \
        as a prefix for deterministic processing. Omit the entry if no ingredient is explicit.
        """
    )
    ...
    @Guide(
        description: """
        Exact source quantity and unit, retaining ranges and preparation qualifiers. Leave empty if missing.
        """
    )
