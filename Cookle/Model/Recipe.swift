...
final class Recipe {
    ...
    init() {
        self.name = Self.randomWords(from: Self.nameList)
        self.imageList = []
        self.ingredientList = (0...Int.random(in: 0..<20)).map { _ in Self.randomWords(from: Self.ingredientList) }
        self.instructionList = (0...Int.random(in: 0..<20)).map { _ in Self.randomWords(from: Self.instructionsList) }
        self.tagList = (0...Int.random(in: 0..<5)).map { _ in Self.randomWords(from: Self.tagList) }
        self.updateDate = .now
        self.creationDate = .now.addingTimeInterval(.random(in: 1...100) * 24 * 60 * 60 * -1)
        self.year = ""
        self.yearMonth = ""
